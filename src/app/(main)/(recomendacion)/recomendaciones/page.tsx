...
// ─── Feedback Content (usado dentro del Modal estándar) ───────────────────────
function FeedbackContent({ rec, onClose }: { rec: Recurso; onClose: () => void }) {
  ...
  const handleEnviar = async () => {
    ...
    try {
      ...
      /* la UI muestra el agradecimiento */
    }
    setSending(false);
    setEnviado(true);
      ...
      <AnimatePresence mode="wait">
        {enviado ? (
          ...
          >
            ...
            <div>
              <h3 style={{ margin: '0 0 6px', fontFamily: 'var(--font-syne)', fontWeight: 700, fontSize: '1.2rem', color: 'var(--text-primary)' }}>
                ¡Gracias por tu valoración!
              </h3>
              <p style={{ margin: 0, fontFamily: 'var(--font-dm-sans)', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                Tu opinión ayuda a mejorar la precisión de las recomendaciones personalizadas.
              </p>
            </div>
          </motion.div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            <p style={{ margin: 0, fontFamily: 'var(--font-dm-sans)', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
              Recurso: <strong style={{ color: 'var(--text-primary)' }}>{rec.titulo}</strong>
            </p>

            {/* Stars */}
            <div>
              <p style={{ margin: '0 0 8px', fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)', fontFamily: 'var(--font-dm-sans)', letterSpacing: '0.06em' }}>
                Calificación
              </p>
                ...
                {[1, 2, 3, 4, 5].map((n) => (
                  <motion.button
                    ...
                    style={{
                      ...
                      color: n <= activeStar ? 'var(--warning, #ffd740)' : 'var(--border-glass)',
                      transition: 'color 0.15s',
                    }}
                  >
                    <Star size={28} fill={n <= activeStar ? 'var(--warning, #ffd740)' : 'transparent'} />
                  </motion.button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {['Muy claro', 'Bien explicado', 'Muy largo', 'Difícil de entender', 'Excelente calidad'].map((tag) => {
                  ...
                  return (
                    <motion.button
                      ...
                      style={{
                        ...
                        background: sel ? 'rgba(59,110,248,0.22)' : 'var(--bg-glass)',
                        border: `1px solid ${sel ? 'var(--accent-blue)' : 'var(--border-glass)'}`,
            ...
            {/* Actions */}
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, paddingTop: 12, borderTop: '1px solid var(--border-glass)' }}>
              <Button variant="ghost" onClick={onClose} disabled={sending}>
                Cancelar
              </Button>
              <Button variant="primary" onClick={handleEnviar} disabled={!canSend} loading={sending}>
                Enviar valoración
              </Button>
            </div>
          </div>
...
// ─── Resource card ─────────────────────────────────────────────────────────────
const RecursoCard = React.forwardRef<HTMLDivElement, {
  recurso: Recurso;
  saved: boolean;
  onSave: () => void;
  onVer: () => void;
  ...
  return (
    ...
    >
      ...
      <Modal
        ...
        title="¿Qué te pareció este recurso?"
        maxWidth={580}
      >
        {valorRecurso && (
          <FeedbackContent rec={valorRecurso} onClose={() => setValorOpen(false)} />
        )}
      </Modal>
